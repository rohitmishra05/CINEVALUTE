...
export class MoviesController {
    ...
    render() {
        this.container.innerHTML = `
            <div class="movies-header" style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: var(--spacing-2xl);">
                <div>
                    <h1 class="animate-slide-up">My Movies</h1>
                    <p class="text-secondary animate-slide-up delay-100">Your entire cinematic journey.</p>
                </div>
                
                <div class="controls animate-slide-up delay-200" style="display: flex; gap: var(--spacing-md);">
                    <select id="movie-filter" class="input-field" style="width: 150px; padding: 0.5rem 1rem;">
                    ...
                    <select id="movie-sort" class="input-field" style="width: 150px; padding: 0.5rem 1rem;">
    ...
    getFilteredAndSortedMovies() {
        ...
        if (this.filter !== 'all') {
            result = result.filter(m => m.status === this.filter);
        }
    ...
    renderGrid() {
        ...
        const displayMovies = this.getFilteredAndSortedMovies();
        ...
        grid.innerHTML = displayMovies.map((movie, index) => {
            const posterUrl = omdb.getImageUrl(movie.poster_path);
            const delay = (index % 10) * 50;
            const statusColor = statusColors[movie.status] || 'var(--text-secondary)';
                ...
                <div class="movie-card animate-scale-up" style="animation-delay: ${delay}ms" data-id="${movie.id}">
                    ...
                    <div class="movie-card-overlay">
                        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                            <h3 class="movie-title-sm" style="flex: 1; white-space: normal; line-height: 1.2;">${movie.title}</h3>
                            ${movie.personalRating > 0 ? `<span style="color: var(--accent-gold); font-size: 0.8rem; font-weight: bold; margin-left: 0.5rem;">⭐ ${movie.personalRating}</span>` : ''}
                        </div>
                        <div style="display: flex; align-items: center; gap: 0.5rem; margin-top: 0.5rem;">
                            <div style="width: 8px; height: 8px; border-radius: 50%; background-color: ${statusColor};"></div>
                            <span style="font-size: 0.75rem; color: var(--text-secondary); text-transform: capitalize;">${movie.status}</span>
                        </div>
                    </div>
                </div>
            `;
        ...
        grid.querySelectorAll('.movie-card').forEach(card => {
            card.addEventListener('click', () => {
                window.location.hash = `#movie-detail/${card.dataset.id}`;
            });
        });
    }

    attachEventListeners() {
        const filterEl = this.container.querySelector('#movie-filter');
        const sortEl = this.container.querySelector('#movie-sort');
